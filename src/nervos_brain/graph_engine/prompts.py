...
3. 最近上下文只用于补全代词、省略和明确追问。被回复消息是直接锚点，但锚点里的澄清项、旧答案和背景不是新的任务清单。
4. 每个 info_need 都必须能说明它如何帮助完成核心交付物；仅仅与对象相关但不影响交付结果的信息不得加入。
5. 先决定是否需要外部事实，再按真正的复杂度选择 single 或 deep；不要按实体名、领域关键词或用户要求“详细”机械升档。
...
- deep 只用于多个相互独立且都由用户明确要求的事实维度、来源冲突、版本差异、复杂排障、跨来源比较或高风险核验。
...
- query 围绕“对象 + 用户动作 + 期望结果”构造，只加入检索所需的必要同义词。
