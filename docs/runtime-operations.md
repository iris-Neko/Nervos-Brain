...
Telegram 还支持可配置的 `/chatid` 诊断命令，用于确认 Bot 当前所在群组和 Topic。建议只给管理员用户开启：
...
管理员可以在群里发送命令，Bot 会把群标题、`chat_id` 和当前 Topic 的 `message_thread_id`（如果有）私聊发给管理员，不会把 ID 返回到群里。命令不进入 Graph，也不受 `allowed_chat_ids` 和 Topic 白名单限制；其它普通消息仍然遵守原有白名单和 mention 规则。`private_response: false` 时才会回到命令所在群聊，只有在受控调试环境中使用。
